...
const CustomCard: React.FC<CustomCardProps> = ({ title, description }) => {
  return (
    <div className="font-roboto bg-white p-8 rounded-lg shadow-md flex flex-col">
      <span className="text-[32px] leading-[36px] tracking-[-0.6px] font-medium font-roboto text-[#3A3A3A]">
        {title}
      </span>
      <div className="border-b border-[#CBD5E1] my-2 w-[80%]"></div>
      <p className="text-[20px] leading-[28px] tracking-[-0.2px] font-normal text-[#334155]">
        {description}
      </p>
    </div>
  );
